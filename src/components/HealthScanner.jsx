...
const HealthScanner = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // 1. Send all data to the Node Backend (which now handles the interlinked Intelligence Layer)
      ...
      // 2. Extract the layered data from your backend
      const apiData = res.data?.data?.result || {};
      const personalized = res.data?.data?.personalizedRecommendation || {}; 
      
      const detectedCalories = apiData.calories || apiData.estimated_calories || 0;

      // Extract the specific recommendation from the new backend intelligence layer, falling back to basic NLP recommendation
      let neuralRecommendation = personalized?.message || personalized?.recommendation || apiData.recommendation || "No recommendation";

      // Map Final UI Result
      const apiResult = {
        detectedItems: apiData.detectedItems || apiData.detected_items || [],
        calories: detectedCalories,
        activityLevel: apiData.activityLevel || apiData.activity_level || "Unknown",
        riskFlags: apiData.riskFlags || apiData.risk_flags || [],
        healthScore: apiData.healthScore || apiData.health_score || 0,
      ...
      {result && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* NEURAL RECOMMENDATION GENERATED BY SECONDARY API */}
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                 <Sparkles className="h-4 w-4 text-primary animate-pulse" />
                 <p className="text-sm font-semibold text-primary">Neural Dietary Recommendation</p>
              </div>
              <p className="font-medium text-sm leading-relaxed whitespace-pre-wrap">
                {result?.recommendation || "Processing localized heuristics..."}
              </p>
            </div>
